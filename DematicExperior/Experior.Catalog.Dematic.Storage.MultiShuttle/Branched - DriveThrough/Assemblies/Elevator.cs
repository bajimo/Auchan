...
namespace Experior.Catalog.Dematic.Storage.MultiShuttle.Assemblies
{
    public class Elevator : Assembly
    {
        ...
        private Core.Timer taskMonitor;
        ...
        private void Scene_OnResetCompleted()
        {
            Core.Environment.Invoke(taskMonitor.Start);
        }

        private void TaskMonitor_OnElapsed(Timer sender)
        {
            SetNewElevatorTask();
        }

        /// <summary>
        /// Complete the current task and search for the next task
        /// </summary>
        public void CompleteTask()
        {
            ...
            CurrentTask = null;
            Log.Write(string.Format("Aisle {0} Elevator {1} - Task Completed", ParentMultiShuttle.Name, Name));
        }

        /// <summary>
        /// Complete the current task, completely reset the elevator and delete all tasks in the task list
        /// </summary>
        public void ResetVehicle()
        {
            Log.Write(string.Format("Aisle {0} Elevator {1} - Reset", ParentMultiShuttle.Name, Name));
            Reset();
        }
                        ...
                        if (blockingTask == null)
                        {
                            ...
                            else if (lowestTask != null)
                            {
                                ...
                                if (((RackConveyor)lowestTask.SourceLoadBConv).LocationA.Active && ((RackConveyor)lowestTask.SourceLoadBConv).LocationA.ActiveLoad.Identification == lowestTask.LoadB_ID)
                                {
                                    ...
                                    var zTasks = ElevatorTasks.Where(task => task.DropIndexLoadB == 0 && task.SourceLoadBConv == lowestTask.SourceLoadBConv && task.Flow == TaskType.Outfeed);
                                    if (zTasks.Any())
                                    {
                    ...
                    else //When the drop station is not available then try to find one on the infeed station
                    {
                        ...
                        if (iTasks.Any())
                        {
                            while (CurrentTask == null)
                            {
                                if (iTasks.Any())
                                {
                                    ElevatorTask topTask = iTasks.First();

                                    if (topTask.Flow == TaskType.HouseKeep &&
                                        ((RackConveyor)topTask.SourceLoadBConv).LocationB.ActiveLoad != null &&
                                        ((RackConveyor)topTask.SourceLoadBConv).LocationB.ActiveLoad.Identification != topTask.LoadB_ID)
                                    {
                                        //If the top task is a house keep move, check that there is no load in front, blocking the path  
                                        ElevatorTask frontTask = iTasks.FirstOrDefault(x => x.SourceLoadBConv == topTask.SourceLoadBConv);
                                        if (frontTask != null)
                                        {
                                ...
                                {
                                    break; //Jump out if there are no more tasks
                                }
                            }
                        }
                    }
        ...
        public ElevatorTask CurrentTask
        {
            ...
            set
            {
                if (value != null)
                {
                    Log.Write(string.Format($"Aisle { ParentMultiShuttle.Name} Elevator { ElevatorName}.Starting elevator task - {value}"));
                    lastTask = currentTask;
                    currentTask = value;
                    Vehicle.TaskDisplay = value.ToString();
                ...
                else if (value == null)
                {
                    ...
                    if (currentTask != null)
                    {
                        Log.Write(string.Format($"Aisle {ParentMultiShuttle.Name} Elevator {ElevatorName}. Done with elevator task - {currentTask}"));
                        PrintTasks();
                    }
                    currentTask = null;
                    Vehicle.TaskDisplay = "";
