...
namespace Experior.Catalog.Dematic.DatcomUK.Assemblies
{
    ...
    public class CasePLC_Datcom : BaseController, IController, ICaseController //TODO Change the name to MHEController_Case
    {
        ...
        {
            if (RoutingTable.ContainsKey(ULID.TrimStart('0')))
            {
                RoutingTable.Remove(ULID.TrimStart('0'));
            }
        }
        ...
        {
            if (!RoutingTable.ContainsKey(barcode.TrimStart('0')) || validRoutes == null || validRoutes.Count == 0)
                return false;

            foreach (int[] wordBit in validRoutes)
            {
                if (GetBit(RoutingTable[barcode.TrimStart('0')], wordBit[1], wordBit[0]))
                    return true;
            }
            return false;
        }
