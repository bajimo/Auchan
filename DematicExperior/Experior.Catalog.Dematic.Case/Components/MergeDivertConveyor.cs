...
namespace Experior.Catalog.Dematic.Case.Components
{
    public class MergeDivertConveyor : Catalog.Logistic.Track.Straight, IRouteStatus, IControllable
    {
        ...
        RouteStatus PreviousAvailableStatusLeft;
        //RouteStatus PreviousAvailableStatusRight;
        ...
        void ReleaseLoadIntoConveyor()
        {
            ...
            //Additional logic for merging on Auchan, only added specific left/straight merge priority currently to speed of deployment
            if (MergePriority != Direction.None)
            {
                //Find the conveyor that the merge routing is connected to...
                if (mergeQueue.Count == 2 && MergePriority == Direction.Straight && mergeQueue[0] == Direction.Left && PreviousAvailableStatusLeft.Available != RouteStatuses.Blocked) //this is the only one that we are interested in for Auchan
                {
                    //re-order the mergeQueue to prioritise straight unless the left route is blocked
                    mergeQueue[0] = Direction.Straight;
                    mergeQueue[1] = Direction.Left;
                }
            }
            ...
            {
                //If there is nothing waiting then clear the waiting  
        ...
        void LeftFixPoint_OnSnapped(FixPoint stranger, FixPoint.SnapEventArgs e)
        {
            ...
            else if (stranger.Type == FixPoint.Types.End)
            {
                ...
                //Added for merge priority (Auchan) only implemented for left route for speed of deployment
                PreviousAvailableStatusLeft = PreviousConveyorLeft.GetRouteStatus(stranger);
            }
        }
        ...
        [Description("Merge loads from this route only of the feeding conveyor is full - Will only apply when used as a merge point")]
        [PropertyOrder(10)]
        public Direction MergePriority
        {
            get { return mergeDivertInfo.mergePriority; }
            set
            {
                mergeDivertInfo.mergePriority = value;
            }
        }
        #endregion
    ...
    public class MergeDivertConveyorInfo : Catalog.Logistic.Track.StraightInfo, IControllableInfo
    {
        ...
        public Direction mergePriority = Direction.None;
